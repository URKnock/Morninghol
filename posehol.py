...
parser.add_argument("-d", "--device", help="Specify the target device to infer on; CPU, GPU, MYRIAD is acceptable. (Default=CPU)", default="MYRIAD", type=str)
...
graph_xml = "models/graph/retrained/yogahol.xml"
graph_bin = "models/graph/retrained/yogahol.bin"
...
net2 = IENetwork(model=graph_xml, weights=graph_bin)
input_blob2 = next(iter(net2.inputs))
exec_net2 = plugin.load(network=net2)
inputs2 = net2.inputs["input"]
...
try:
    while True:
        ...
        cv2.imwrite('./temp.jpg', frameClone)

        prepimg2 = read_tensor_from_image_file('./temp.jpg', 224, 224, 128, 128)
        prepimg2 = prepimg2.transpose((0, 3, 1, 2))

        outputs2 = exec_net2.infer(inputs={input_blob2: prepimg2})["final_result"]
        results = np.squeeze(outputs2)

        top_k = results.argsort()[-5:][::-1]
        labels = load_labels("models/graph/retrained/yogahol.txt")
        ...
        for i in top_k:
            try :
                ...
                print("ERROR!\n")
        pose_result = "Pose : %s" %(template.format(labels[top_k[0]], results[top_k[0]]))
 
        cv2.putText(frameClone, fps, (w-170,15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1, cv2.LINE_AA)
        cv2.putText(frameClone, pose_result, (10, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1, cv2.LINE_AA)
 
        cv2.namedWindow("morninghol", cv2.WINDOW_AUTOSIZE)
        cv2.imshow("morninghol" , frameClone)
